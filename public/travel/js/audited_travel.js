$(function(){
    ...
    if(uuId){
        var gettravel = '/travelapplication/getTravelApplication/'+ uuId ;
    } else{
        var gettravel = '/travelapplication/getUserInfo'
    }
    init(gettravel);
 		...
 		$.ajax({ 
            ...
            success: function (data) {
            	...
            	if(data.resultCode == 0){
            		...
            		localStorage.setItem("fdOrgSid", data.result.travelApplication.fdOrgSid );
            		localStorage.setItem("fdSid", data.result.travelApplication.fdSid );
...
//			contentType : 'application/json;charset=UTF-8',
	...
	$('body').on('click', '.rank_change_click',function(){
		$('.rank_change').css('display','block')
	});
	$('body').on('click','.rankchange_ul li', function(){
		console.log($(this).children('.rankright').children('.no_selected').attr('src'))
		$(this).siblings().children('.rankleft').children('h3').removeClass('changetypeval'); 
		$(this).children('.rankleft').children('h3').addClass('changetypeval');
		$(this).siblings().children('.rankright').children('.no_selected').attr('src','../../img/no_selected.png');
		$(this).children('.rankright').children('.no_selected').attr('src','../../img/selected.png');  
		var changetypeval =$(this).children('.rankleft').children('.changetypeval').html();
		console.log(changetypeval)
		$('.changeTypeVal').html(changetypeval);
	})
 ...
 // 校验天数
   	$('.count').click(function(){
	    ...
	    if(loginName){
	    	$.ajax({ 
	        	url: sy() + "/travelapplication/documentChange/"+ uuId +"/"+datetimestart+"/"+ datetimeend,
				type : 'GET',
				dataType: 'json',
				contentType : 'application/json;charset=UTF-8',
	            success: function (data) {
	...
	$('#next_step').click(function(){
		hrefUrl = 'audited_newApply.html?uuId='+ uuId;
...
//		var businessLineType     = $('.rankTypelinear').html();//线性类型
		...
		var rankchange         = $('.changeTypeVal').html();
		if(businessLineType){
        ...
        var dataobj={   
                    "id" : uuId,
                    "businessTripMoney": businessTripMoney ,//出差总金额
...
//					if(parseFloat(businessTripMoney) < parseFloat(businessSurplusMoney)){
						$(".loading").fadeIn();
						$.ajax({ 
							url: sy + 'travelapplication/saveChangeTravelapplication',
							type : 'POST',
							dataType: 'json',
							contentType : 'application/json;charset=UTF-8',
