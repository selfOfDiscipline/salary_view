...
//	$('.traveldaysVal').attr('placeholder',0)
	...
	function init(){
		$.ajax({
        	url: sy() + "/travelapplication/getTravelapplicationDetail/" + Id ,
			type : 'GET',
			dataType: 'json',
			async: false,
//			contentType : 'application/json;charset=UTF-8',
...
//	        		alert(JSON.parse(sessionStorage.getItem("deptListsel")).length)
	...
	$('#next_step').click(function(){
		...
		text ="提交";
		Url = sy + 'travelflow/travelapplication/1' ;
	...
	if(rejectedType == '1'){
		...
		$('.identity_wrap').css('pointer-events','none');
	}
